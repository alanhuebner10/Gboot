#' Create linear random effects model design matrix for simulated data
#'
...
#' @return A design matrix which is then given to the sim_study_2crossed() function
#' @export
construct_DesMat_2crossed <- function(np, ni, nr) {
  ...
  sim_data$Fac1 <- rep(1:ni, each = nr, times = np)
  sim_data$Fac2 <- rep(1:nr, times = np*ni)
  return(sim_data)
}

...
#' @param sig_pio Generating (i.e. population) value for residual standard deviation.
#'
...
#'                  sig_pi = 8, sig_pr = sqrt(2), sig_ir = sqrt(3), sig_pio = 12)
#'
#' }
...
sim_dat_2crossed <- function(np, ni, nr, sig_p, sig_i, sig_r,
  ...
  for (p in 1:np) {
    for (i in 1:ni) {
      for (r in 1:nr) {
        Score_pio <- sig_p*Zp[p] + sig_i*Zi[i] + sig_r*Zr[r] +
          sig_pi*Zpi[p,i] + sig_pr*Zpr[p,r] + sig_ir*Zir[i,r] + sig_pio*rnorm(1)
        sim_data$Score[score_index] <- Score_pio
